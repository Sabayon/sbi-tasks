...
TREE_PATH="${TREE_PATH:-../../../../mocaccino/mocaccino-portage-tree/amd64}"
COMMON_TREE_PATH="${COMMON_TREE_PATH:-../../../../mocaccino/mocaccino-portage-tree/multi-arch}"
CONCURRENCY=${CONCURRENCY:-1}
TASK_NAME_PREFIX="${TASK_NAME_PREFIX:-AMD64 mocaccinoos/mocaccino-portage-tree: }"
LUET_OPTS="${LUET_OPTS:-}"
MCLI_OPTS="${MCLI_OPTS:-}"
REPO_URL="${REPO_URL:-https://github.com/mocaccinoOS/portage-tree.git}"
...
push_task () {
  ...
  mottainai-cli task compile build-pkgs.tmpl \
    ...
    -s LuetPkgs="$list" \
    -s RepoName="mocaccino-portage-tree" \
    -s RepoUrl="${REPO_URL}" \
