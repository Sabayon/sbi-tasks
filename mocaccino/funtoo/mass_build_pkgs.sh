...
REPO_PATH="${REPO_PATH:-../../../../mocaccino/mocaccino-funtoo/}"
CONCURRENCY=${CONCURRENCY:-1}
TASK_NAME_PREFIX="${TASK_NAME_PREFIX:-AMD64 mocaccinoos/funtoo: }"
LUET_OPTS="${LUET_OPTS:-}"
MCLI_OPTS="${MCLI_OPTS:-}"
TREE_OPTS="${TREE_OPTS:---tree ${REPO_PATH}/amd64/packages --tree ${REPO_PATH}/multi-arch/packages}"
...
n_pkgs=$(${LUET_BIN} --config ${LUET_CONFIG} tree pkglist ${TREE_OPTS} ${LUET_OPTS} | wc -l)
pkgs=$(${LUET_BIN} --config ${LUET_CONFIG} tree pkglist ${TREE_OPTS} ${LUET_OPTS})
