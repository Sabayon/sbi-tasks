...
TREE_PATH="${TREE_PATH:-../../../luet-repo/}"
CONCURRENCY=${CONCURRENCY:-1}
TASK_NAME_PREFIX="${TASK_NAME_PREFIX:-Mocaccino ARM luet: }"
...
n_pkgs=$(${LUET_BIN} --config ${LUET_CONFIG} tree pkglist --tree ${TREE_PATH}/packages ${EXCLUDES} -v | wc -l)
pkgs=$(${LUET_BIN} --config ${LUET_CONFIG} tree pkglist --tree ${TREE_PATH}/packages ${EXCLUDES} -v)
...
push_task () {
  ...
  if [ -z "${SKIP_FIRE}" ] ; then
    mottainai-cli task create --yaml /tmp/luet-repo/task_${n_task}.yaml
  fi
}
